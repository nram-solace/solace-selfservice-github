...
def main(argv):
    ...
    # Read the per-environment inventory file: inventory/<client>/<env>.yml
    client = run_params.get('client')
    env = run_params.get('env')
    app_id = run_params.get('app')
    if not client or not env or not app_id:
        print ('{}[{}]{} ERROR: params must contain client, env and app (generated by csv_to_yaml.py)'.format(
            CYAN, SCRIPT_PREFIX, NC))
        sys.exit(1)
    print ('{}[{}]{} Reading inventory {}/{}/{}.yml (app: {})'.format(
        CYAN, SCRIPT_PREFIX, NC, system_cfg['inventoryDir'], client, env, app_id))
    inv = Inventory.Inventory(cfg, r.verbose)
    inv_data = inv.read_inventory(system_cfg['inventoryDir'], client, env)
    ...
    log.info ('INVENTORY     : {} apps'.format(len(inv_data)))
    ...
    if app_id not in inv_data:
        log.error ('App: {} not found in inventory {}/{}/{}.yml'.format(
            app_id, system_cfg['inventoryDir'], client, env))
    ...
    # The env name comes from params.env (the <env> directory / inventory file name),
    # upper-cased with non-alphanumeric chars mapped to '_' to match GitHub secret
    # naming (e.g. env 'sys' -> $SEMP_SYS_PASSWORD).
    env_pw_var = 'SEMP_{}_PASSWORD'.format(
        ''.join(c if c.isalnum() else '_' for c in str(env)).upper())
